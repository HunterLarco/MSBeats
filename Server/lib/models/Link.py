...
class Link(ndb.Model):
  ...
  @classmethod
  def queryByUser(cls, user, count=30, page=1):
    return cls.query(cls.userkey == user.key).order(-cls.created).fetch(count, offset=(page-1)*count)

  @classmethod
  def queryNew(cls, count=30, page=1):
    return cls.query().order(-cls.created).fetch(count, offset=(page-1)*count)

  @classmethod
  def queryTop(cls, count=30, page=1):
...
class TrendingCounter(ndb.Model):
  ...
  @classmethod
  def queryTrending(cls, page=1, count=30):
    return map(lambda x: x.getLink(), cls.query().order(-cls.score).fetch(count, offset=(page-1)*count))
